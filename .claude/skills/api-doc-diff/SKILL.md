...
| 2 | `tests/diff/doc/` | 代码变更文档（模板参考 `.claude/references/代码变更说明文档模板.md`） |
...
仅处理受影响实体和被引用实体。按 `.claude/references/实体类文档模板.md` 格式生成。
...
仅处理受影响的模块。按 `.claude/references/模块文档模板.md` 格式生成。
...
注意：
- **断言生成（核心规则）**：代码变更文档中有"测试要点"的接口，必须在接口段中生成"#### 断言"节。断言是按项目断言规范（`.claude/rules/testCase-standards.md` 断言-协议层/业务层/数据层）将测试要点转化为结构化验证规格，供下游 `api-doc-to-testcases` 直接映射为 YAML `expected` 块。
