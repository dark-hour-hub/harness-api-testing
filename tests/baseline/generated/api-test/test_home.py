"""
test_home.py --- 由 yaml-to-pytest skill 自动生成，请勿手动修改
...
_YAML_FILE = _YAML_DIR / "01-首页.yaml"
...
@pytest.mark.order(1)
def test_TC_HOME_001(module_context, request_helper):
    """GET /_访问欢迎页_返回HTML页面"""
    case = _get_case("TC_HOME_001")
    resp, data = request_helper.execute(case)
    request_helper.store_extracts("TC_HOME_001", case, data)

@pytest.mark.order(2)
def test_TC_HOME_002(module_context, request_helper):
    """GET /_服务器内部错误_返回错误页面"""
    case = _get_case("TC_HOME_002")
    resp, data = request_helper.execute(case)
    request_helper.store_extracts("TC_HOME_002", case, data)
