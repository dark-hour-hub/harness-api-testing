...
| 字段规范 | `.opencode/skills/api-doc-to-testcases/references/yaml-schema.md` | YAML 字段含义 |
| 变量引用规范 | `.opencode/skills/api-doc-to-testcases/references/variable-reference.md` | `${...}` 解析规则 |
...
```bash
# 全量模式（默认）
python .opencode/skills/yaml-to-pytest/scripts/generate_pytest.py --mode baseline

# 增量模式
python .opencode/skills/yaml-to-pytest/scripts/generate_pytest.py --mode diff

# 显式指定路径（优先级高于 --mode）
python .opencode/skills/yaml-to-pytest/scripts/generate_pytest.py \
