...
def _case_meta(tc: dict) -> dict:
    """提取用例元数据（供报告展示标题/步骤/预期/断言）"""
    return {
        "id": tc.get("id"),
        "title": tc.get("title", ""),
        ...
        "extracts": tc.get("extracts", {}),
        "depends_on": tc.get("depends_on", []),
    }


def write_cases_meta(meta_map: dict, output_dir):
    """写聚合用例元数据文件 _cases_meta.json（key 为 case_id，供报告生成器关联）"""
    meta_path = Path(output_dir) / "_cases_meta.json"
...
def main():
    ...
    for yf in yaml_files:
        ...
        try:
            ...
            for tc in ydata.get("testcases", []):
                if tc.get("id"):
                    meta = _case_meta(tc)
                    meta["module"] = ydata.get("module", yf.stem)
                    meta["module_name"] = ydata.get("module_name", ydata.get("module", yf.stem))
                    all_meta[meta["id"]] = meta
        except Exception as e:
            print(f"  WARN: 读取 {yf.name} 元数据失败: {e}")

    if all_meta:
        meta_path = write_cases_meta(all_meta, output_dir)
        print(f"\n[OK] 用例元数据: {meta_path} ({len(all_meta)} 个用例)")
