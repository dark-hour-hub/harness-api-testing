...
def _case_id_from_name(name: str) -> str:
    """'test_TC_AGENT_001' / 'test_TC_AGENT_001[chromium]' -> 'TC_AGENT_001'"""
    cid = name.strip()
...
def load_cases_meta(test_dir: str) -> dict:
    """从 yaml-to-pytest 生成的 _cases_meta.json 加载用例元数据"""
    path = os.path.join(test_dir, "_cases_meta.json")
...
def enrich_cases_with_meta(cases: list, meta_map: dict) -> None:
    """把 YAML 用例元数据（标题/描述/步骤/预期）挂到每个 case dict"""
    for case in cases:
        cid = _case_id_from_name(case["name"])
        case["case_id"] = cid
        case["meta"] = meta_map.get(cid) or {}


def _build_case_detail_html(case: dict) -> str:
    """生成用例详情面板：说明 / 步骤 / 预期断言 / 提取 / 依赖 / 实际结果"""
    meta = case.get("meta") or {}
    parts = []

    if meta.get("description"):
        parts.append(
            f'<div class="d-section"><div class="d-title">用例说明</div>'
            f'<div class="d-desc">{_html_escape(meta["description"])}</div></div>')

    step_items = []
    method = (meta.get("method") or "").upper()
    path = meta.get("path", "")
    if method or path:
        step_items.append(f'<li><b class="d-req">{_html_escape(method)}</b> {_html_escape(path)}</li>')
    req = meta.get("request") or {}
    if req.get("path_params"):
        step_items.append(f'<li>路径参数<pre>{_html_escape(_pretty_json(req["path_params"]))}</pre></li>')
    if req.get("query"):
        step_items.append(f'<li>Query 参数<pre>{_html_escape(_pretty_json(req["query"]))}</pre></li>')
    if req.get("headers"):
        step_items.append(f'<li>请求头<pre>{_html_escape(_pretty_json(req["headers"]))}</pre></li>')
    if req.get("body"):
        step_items.append(f'<li>请求体<pre>{_html_escape(_pretty_json(req["body"]))}</pre></li>')
    if step_items:
        parts.append(f'<div class="d-section"><div class="d-title">执行步骤</div>'
                     f'<ul class="d-steps">{"".join(step_items)}</ul></div>')

    expected = meta.get("expected") or {}
    if expected:
        exp_rows = "".join(
            f'<tr><td class="d-k">{_html_escape(str(k))}</td>'
            f'<td><pre>{_html_escape(_pretty_json(v))}</pre></td></tr>'
            for k, v in expected.items())
        parts.append(f'<div class="d-section"><div class="d-title">预期与断言</div>'
                     f'<table class="d-table">{exp_rows}</table></div>')

    extracts = meta.get("extracts") or {}
    if extracts:
        parts.append(f'<div class="d-section"><div class="d-title">提取变量</div>'
                     f'<pre>{_html_escape(_pretty_json(extracts))}</pre></div>')

    deps = meta.get("depends_on") or []
    if deps:
        parts.append(f'<div class="d-section"><div class="d-title">前置依赖</div>'
                     f'<div class="d-desc">{"、".join(_html_escape(d) for d in deps)}</div></div>')

    if case["status"] == "passed":
        actual = f'<div class="d-actual pass">&#10003; 通过（耗时 {case["time"]:.2f}s）</div>'
    else:
        msg = case.get("message") or ("跳过原因" if case["status"] == "skipped" else "失败")
        actual = f'<div class="d-actual fail">&#10007; {_html_escape(msg)}</div>'
        if case.get("trace"):
            actual += f'<pre class="d-trace">{_html_escape(case["trace"])}</pre>'
    resp = case.get("response")
    if resp:
        sc = resp.get("status_code")
        body = resp.get("body") or ""
        actual += (f'<div class="d-resp"><span class="d-resp-code">HTTP {_html_escape(sc)}</span>'
                   f'<pre>{_html_escape(body)}</pre></div>')
    parts.append(f'<div class="d-section"><div class="d-title">实际结果</div>{actual}</div>')
        ...
        for tc in ts.findall("testcase"):
            ...
            if props is not None:
                for prop in props.findall("property"):
                    if prop.attrib.get("name") == "api_response":
                        try:
                            case["response"] = json.loads(prop.attrib.get("value", "{}"))
                        except Exception:
                            case["response"] = None
...
def build_detail_rows(cases: list[dict]) -> str:
    """生成详细结果表格行 HTML（含用例标题 + 步骤/预期/断言详情面板）"""
    rows = []
    for i, case in enumerate(cases):
        ...
        meta = case.get("meta") or {}
        title = meta.get("title") or case["name"]
        case_id = case.get("case_id", "")
        detail = _build_case_detail_html(case)
        # 失败/错误默认展开第一条详情
        default_open = i == 0 and status in ("failed", "error")
        display = "table-row" if default_open else "none"
        ...
        rows.append(f"""
            <tr class="case-row status-{status}" onclick="toggleTrace(this)">
                ...
                <td class="col-name" title="{_html_escape(title)}">
                    <div class="case-title">{_html_escape(title)}</div>
                    <div class="case-sub">{_html_escape(case_id)}</div>
                </td>
                <td class="col-time">{case['time']:.2f}s</td>
                <td class="col-action">&#9660; 详情</td>
            </tr>
            <tr class="trace-row" style="display:{display};">
                <td colspan="4">
                    <div class="trace-box case-detail">
                        {detail}
                    </div>
                </td>
            </tr>""")
...
def main():
    ...
    # ── Step 2.5: 关联用例元数据（标题/步骤/预期，来自 yaml-to-pytest 生成的 _cases_meta.json）──
    meta_dir = test_path if os.path.isdir(test_path) else os.path.dirname(test_path)
    meta_map = load_cases_meta(meta_dir)
    if meta_map:
        enrich_cases_with_meta(data["cases"], meta_map)
        print(f"[run_tests] 已关联用例元数据: {len(meta_map)} 个用例")
